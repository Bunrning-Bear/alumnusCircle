...
def send_file(org, app, auth, file_path):
    ...
    files = {'file': ('report.xls', open(file_path, 'rb'),
...
if __name__ == '__main__':
    ...
    app_client_auth = AppClientAuth(
        org_name, app_name, test_client_id, test_client_secret)
    ...
    for i in range(10):
        ...
        test_success, test_result = register_new_user(
            org_name, app_name, app_client_auth, test_username, test_password)
        if test_success:
    ...
    for test_username in app_users:
        test_success, test_result = delete_user(
            org_name, app_name, app_client_auth, test_username)
        if test_success:
    ...
    test_success, test_result = send_file(
        org_name, app_name, app_client_auth, '../zjg.jpg')
