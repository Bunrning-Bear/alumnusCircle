...
class UploadNormalImgHandler(UploadImgHandler):
    ...
    def get(self):
        try:
            result = json.dumps({
                "code": 74138, "message": "Hello, Welcome to upload your image !", "img_key": "empty", "img_url": "empty"
            })
            self.write(result)
        except:
            pass
    ...
    # without local cache, upload to cloud server directly.(This is a better
    # choice)
    @tornado.web.asynchronous
    @tornado.gen.coroutine
    def post(self):
        try:
            ...
            img_url = self.aliyun.parseUrlByKey(key)
            result = json.dumps({
                "code": 600, "message": "Upload Image Successfully !", "img_key": fakeKey, "img_url": img_url
            })
            self.write(result)
        except Exception, e:
            result = json.dumps({
                "code": 601, "message": "I'm sorry about that you are faild to upload image.", "img_key": "empty", "img_url": "empty"
            })
            self.write(result)
        finally:
            self.finish()
